...
public interface IMessageSocketRepository extends JpaRepository<Message, Long> {
    ...
    @Query(value = "select m.id_sender as id_sender, sender.name as name from messages m join accounts as sender on m.id_sender = sender.id join accounts as receiver on m.id_receiver = receiver.id where m.id_receiver = ?1 UNION select m.id_receiver as id_sender, receiver.name as name from messages m join accounts as sender on m.id_sender join accounts as receiver on m.id_receiver = receiver.id where m.id_sender = ?2", nativeQuery = true)
